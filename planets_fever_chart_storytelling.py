...
# 🌍 Planet’s Fever Chart — Climate Data Storytelling (NASA GISTEMP)
...
- Craft a short narrative: *Setup → Conflict → Resolution*.
"""
...
| Column      | Meaning                                    |
| ----------- | ------------------------------------------ |
| **Year**    | The calendar year (1880–2025).             |
| **Jan–Dec** | Monthly global temperature anomalies (°C). |
| **J–D**     | Annual mean (January–December).            |
| **D–N**     | Annual mean (December–November).           |
| **DJF**     | Winter (December–February) seasonal mean.  |
| **MAM**     | Spring (March–May) mean.                   |
| **JJA**     | Summer (June–August) mean.                 |
| **SON**     | Autumn (September–November) mean.          |
...
(if using a centered window of 10 years; sometimes it’s 5 before + 4 after, or trailing average).

So, for example, the “smoothed” value for 2015 is the mean of anomalies from roughly 2010–2019.

# You have two threshold choices, plus a “None” option:

| Label       | Value  | Meaning                                                                                                                                                                                                       |
| ----------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **+1.5 °C** | `1.5`  | Represents the aspirational global warming limit set by the **Paris Agreement (2015)** — the world aims to limit temperature rise to **no more than +1.5 °C above pre-industrial levels**.                    |
| **+2.0 °C** | `2.0`  | Represents the **upper boundary** of “acceptable” warming — beyond which the risk of severe and irreversible climate impacts increases sharply. This is the **formal maximum target** in the Paris Agreement. |
| **None**    | `None` | Disables the threshold line — used if you want to view anomalies without any reference limit.                                                                                                                 |
...
# === Cell 8: Radial donut wall: 1951–2025, 15 per row × 5 rows (bigger donuts) ===
...
# 3) Fix the target years: 1951–2025 (75 years = 5 rows × 15)
target_start = 1951
...
# 5) Figure layout: 15 cols × 5 rows; bigger donuts
ncols, nrows = 15, 5
...
for idx, yr in enumerate(target_years):
    ...
    if yr in available_years:
        ...
        # Whole year missing → leave all wedges blank
        vals = {m: np.nan for m in range(1,13)}
...
# Title & margins
fig.suptitle("Monthly global mean temperature anomalies 1951–2025 (vs 1951–1980)", y=0.995, fontsize=18)
...
fig.add_hline(y=0, line_dash="dash")
fig.update_layout(title="Regional Anomalies (Annual, J–D)", xaxis_title="Year",
...
# Builds anomalies vs 1951–1980 and absolute annual temperature maps.
...
def load_one(path):
    ...
    if not REQUIRED.issubset(set(df.columns)):
        print(f"⚠️ Skipping {country}: needed {REQUIRED}, found {list(df.columns)[:8]}")
        return None
    ...
    if df.empty:
        print(f"⚠️ {country}: no rows after cleaning.")
        return None
...
# Per-country 1951–1980 baseline & anomalies
...
# Keep countries with reasonable baseline coverage (>= 20 years in 1951–1980)
...
if unmatched:
    print("⚠️ Unmatched country names (first 15):", unmatched[:15])
...
# === Animated choropleth — Anomaly (1880–2020) ===
...
# === Animated choropleth — Absolute (°C) (1880–2020) ===
...
if year_exceed_1_5:
    print(f"\n⚠️  CRITICAL: 1.5°C threshold will be EXCEEDED in {year_exceed_1_5}")
    print(f"    Temperature at crossing: {temp_at_exceed:.3f}°C")
else:
    print("\n✓ 1.5°C threshold not exceeded in forecast period")
...
print("✓ Forecast data exported to 'temperature_forecast_2100.csv'")
print("✓ Plot saved as 'temperature_forecast_2100.png'")
...
ax.set_title('Atmospheric CO₂ Concentrations at Mauna Loa Observatory\n(1959-2024)',
...
stats_text += f'{df["year"].iloc[0]}: {start_co2:.2f} ppm → {df["year"].iloc[-1]}: {end_co2:.2f} ppm'
...
print(f"\nKey Statistics:")
print(f"Starting CO₂ ({df['year'].iloc[0]}): {start_co2:.2f} ppm")
print(f"Ending CO₂ ({df['year'].iloc[-1]}): {end_co2:.2f} ppm")
...
"""### ✍️ Your Data Story Task(2–4 sentences)
